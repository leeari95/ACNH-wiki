...
pre-push 훅이 소스 변경 시 관련 `docs/` 업데이트를 요구함. push 실패 시:

1. 에러 메시지에 표시된 관련 문서 파일을 업데이트
2. 문서 변경을 별도 커밋으로 생성 (`📝 [docs] ...`)
3. 다시 push 실행

**사용자에게 물어보지 않고 자동으로 처리**할 것. 문서 업데이트 → 커밋 → 재push까지 한 번에 진행.
