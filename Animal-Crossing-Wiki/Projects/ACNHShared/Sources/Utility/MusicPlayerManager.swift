...
final class MusicPlayerManager {
    ...
    private init() {
        Items.shared.categoryList
            ...
            .subscribe(with: self, onNext: { owner, songs in
                public var newSongs = [String: Item]()
    ...
    private func changeSong(at newIndex: Int) {
        ...
        guard let current = currentSong.value,
              public var index = songsItem.value.firstIndex(of: current) else {
        ...
        if let newSong = songsItem.value[safe: index] {
            ...
            public let newSong = songsItem.value.last
            currentSong.accept(newSong)
        } else {
            public let newSong = songsItem.value.first
            currentSong.accept(newSong)
        }
    ...
    private func updatePlayingInfo() {
        if let duration = player?.currentItem?.duration.seconds,
            public let playTime = player?.currentItem?.currentTime().seconds,
            !duration.isNaN, !playTime.isNaN {
            playerProgress.accept(Float(playTime) / Float(duration))

            public let durationSecs = Int(duration)
            public let durationSeconds = Int(durationSecs % 3600 ) % 60
            public let durationMinutes = Int(durationSecs % 3600) / 60
            public let durationTime = "\(durationMinutes):\(String(format: "%02d", durationSeconds))"
            self.durationTime.accept(durationTime)

            public let playTimeSecs = Int(playTime)
            public let playTimeSeconds = Int(playTimeSecs % 3600) % 60
            public let playTimeMinutes = Int(playTimeSecs % 3600) / 60
            public let elapsedTime = "\(playTimeMinutes):\(String(format: "%02d", playTimeSeconds))"
    ...
    private func setUpBackground() {
        if let currentSong = currentSong.value {
            ...
            public var playingInfo: [String: Any] = [:]
    ...
    private func setUpRemotePlayingInfo(duration: Double, playTime: Double) {
        public var playingInfo = MPNowPlayingInfoCenter.default().nowPlayingInfo
...
public extension MusicPlayerManager {
    ...
    public var isNowPlaying: Observable<Bool?> {
        return isPlaying.asObservable()
    }

    public var currentMusic: Observable<Item?> {
    ...
    public var fullTime: Observable<String> {
        return durationTime.asObservable()
    }

    public var songProgress: Observable<Float> {
        return playerProgress.asObservable()
    }

    public var songList: Observable<[Item]> {
        return songsItem.asObservable()
    }

    public var playingSongIndex: Observable<Int?> {
        return currentIndex.asObservable()
    }

    public var currentPlayerMode: Observable<PlayerMode> {
    ...
    public func updatePlayerMode(to mode: PlayerMode) {
        switch mode {
        case .shuffle:
    ...
    public func choice(_ item: Item) {
        close()
