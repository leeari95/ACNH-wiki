...
final class Items {
    ...
    public var list: [Item] { allItems.value }

    public var fixedVisitNpcs: Driver<[NPC]> { fixedVisitNPCList.asDriver() }
    public var randomVisitNpcs: Driver<[NPC]> { randomVisitNPCList.asDriver() }
    ...
    private init() {
        ...
        npc.subscribe(with: self) { owner, list in
            public let randomVisitNPCList = list.filter(\.isRandomVisit)
            public let fixedVisitNPCList = list.filter(\.isFixedVisit)
    ...
    private func setUpUserCollection() {
        ...
        CoreDataItemsStorage().fetch()
            .subscribe(onSuccess: { items in
                public var userItems = [Category: [Item]]()
                items.forEach { item in
                    public var items = userItems[item.category] ?? []
                    items.append(item)
    ...
    private func fetchAnimals() {
        ...
        fetchItem(VillagersRequest(), group: group) { [weak self] response in
            public let items = response.map { $0.toDomain() }
    ...
    private func fetchCritters() {
        networkGroup.enter()
        public let group = DispatchGroup()
        public var itemList: [Category: [Item]] = [:]
    ...
    private func updateAllItemList(by items: [Category: [Item]]) {
        public var currentItems = self.categories.value
        public var itemsCount = self.currentItemsCount.value
    ...
    private func setUpMaterialsItems() {
        public let values = allItems.value
            .filter { $0.category == .recipes }
            .compactMap { $0.recipe?.materials.map { $0.key.description } }
        public let materials = Array(Set(values.flatMap { $0 }))
        public let materialsItems = allItems.value
...
public extension Items {
    ...
    public var villagerList: Observable<[Villager]> {
        return villagers.asObservable()
    }

    public var villagerHouseList: Observable<[Villager]> {
        return villagersHouse.asObservable()
    }

    public var villagerLikeList: Observable<[Villager]> {
        return villagersLike.asObservable()
    }
    
    public var npcList: Observable<[NPC]> {
        return npc.asObservable()
    }

    public var npcLikeList: Observable<[NPC]> {
        return npcLike.asObservable()
    }

    public var categoryList: Observable<[Category: [Item]]> {
        return categories.asObservable()
    }
    public var isLoading: Observable<Bool> {
        return isLoad.asObservable()
    }
    public var userInfo: Observable<UserInfo?> {
        return currentUserInfo.asObservable()
    }

    public var dailyTasks: Observable<[DailyTask]> {
        return currentDailyTasks.asObservable()
    }

    public var itemList: Observable<[Category: [Item]]> {
    ...
    public func count(isItem: Bool = true) -> Observable<[Category: Int]> {
        return (isItem ? currentItemsCount : currentAnimalCount).asObservable()
    }

    public func updateTasks(_ task: DailyTask) {
        public var tasks = currentDailyTasks.value
    ...
    public func deleteTask(_ task: DailyTask) {
        public var tasks = currentDailyTasks.value
        if let index = tasks.firstIndex(where: { $0.id == task.id }) {
    ...
    public func updateVillagerHouse(_ villager: Villager) {
        public var villagers = villagersHouse.value
    ...
    public func updateVillagerLike(_ villager: Villager) {
        public var villagers = villagersLike.value
        if let index = villagers.firstIndex(where: {$0.name == villager.name}) {
            villagers.remove(at: index)
        } else {
            villagers.append(villager)
    ...
    public func updateNPCLike(_ npc: NPC) {
        public var npcLikeList = npcLike.value
    ...
    public func updateItem(_ item: Item) {
        public var currentUserItems = userItems.value
        public var items = currentUserItems[item.category] ?? []
    ...
    public func itemFilter(keyword: String, category: Keyword) -> [Item] {
        public let items = allItems.value
        return items
    ...
    public func reset() {
        ...
        public let currentItems = userItems.value.map { $0.key }
        public var resetItem = userItems.value
    ...
    public func allCheckItem(category: Category) {
        public var items = userItems.value
        public let allItems = categories.value[category]
        public var newItems: [Item]
    ...
    public func resetCheckItem(category: Category) {
        public var items = userItems.value
