...
public extension UIImage {
    public func resizedImage(Size sizeImage: CGSize) -> UIImage? {
        public let frame = CGRect(origin: CGPoint.zero, size: CGSize(width: sizeImage.width, height: sizeImage.height))
        UIGraphicsBeginImageContextWithOptions(frame.size, false, 0)
        self.draw(in: frame)
        public let resizedImage: UIImage? = UIGraphicsGetImageFromCurrentImageContext()
